...
deploy_zip() {
    ...
    if [[ "$CLEAR_KEYS" == true ]]; then
        ...
        adb shell "rm -rf /data/misc/the_next/persistent_keys/*" 2>/dev/null || true
    fi
...
verify_device() {
    ...
    local tee_status
    tee_status=$(adb shell "cat /data/misc/the_next/tee_status.txt" 2>/dev/null || echo "N/A")
    echo "    TEE status: $tee_status"

    local sec_patch
    sec_patch=$(adb shell "cat /data/misc/the_next/security_patch.txt" 2>/dev/null || echo "N/A")
